...
static const dacsample_t dac_null_buffer[DAC_BUFFER_SIZE];
...
static void end_cb1(DACDriver *dacp)
{
    ...
    {   
        nz++;
        dacPutChannelX(&DACD2, (dacchannel_t) 1, (dacsample_t)(16*nz % 4096));
        if(nz%2){
        ...
        else
        {
            dacStopConversion(&DACD1);
            dacStartConversion(&DACD1, &dacgrpcfg1,
                       (dacsample_t *)dac_null_buffer, DAC_BUFFER_SIZE);
        }
